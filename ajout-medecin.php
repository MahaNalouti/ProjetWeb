...
if(isset($_POST['ajoutM'])){
    ...
    !empty($Cmdp) && !empty($sal) && !empty($spec) ){
        ...
        $sql="SELECT COUNT(*) AS count FROM doctor WHERE unom = :unom";
        ...
        try{
            ...
            if($result['count']>0){
                $_SESSION['message']="Nom utilisateur déjà pris";
            } else {
                $sql_spec="SELECT COUNT(*) AS count FROM doctor WHERE specialite = :specialite";
                ...
                if($result_spec['count']>0){
                    ...
                    $sql_insert = "INSERT INTO doctor (nom,prenom,unom,email,tel,sexe,mdp,salaire,specialite) 
                    ...
                    $stmt_insert->bindParam(':sal',$sal);
                    $stmt_insert->bindParam(':specialite',$spec);

                    try{
                        $stmt_insert->execute();
                        $_SESSION['message']="Médecin ajouté";
                    } catch(PDOException $e){
                        $_SESSION['message']="Erreur lors de l'ajout du médecin:".$e->getMessage();
		...
		<main>
			...
			<div class="data">
				...
				<div class="content-data">
					<div class="head">
						<h3>Formulaire</h3>
						<a href="doctor.php" class="btn-green float-end">Retourner</a>
					</div>
						<div>
